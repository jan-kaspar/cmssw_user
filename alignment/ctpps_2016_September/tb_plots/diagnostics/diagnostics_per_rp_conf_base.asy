...
for (int di = 0; di < dirs.length; ++di)
{
	...
	NewPad("$\log_{10}(\ch^2 / \hbox{ndf})$");
	scale(Linear, Log);
	for (int rpsi : rpSets.keys)
	{
		pen p = rps_pens[rpsi];

		RootObject obj = RootGetObject(f, "common/plots per RP set/" + rpSets[rpsi] + "/chi^2 norm, log, selected", error=true);
		if (obj.valid)
			draw(obj, "vl", p);
	}

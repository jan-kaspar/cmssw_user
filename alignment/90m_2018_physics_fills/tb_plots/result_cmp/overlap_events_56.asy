...
string postfix = "/103,104,105,123,124,125/s,std,3pl,1rotzIt=0,units=2,overlap=t,3potsInO=t,eMuMvRot=f/iteration1";
...
for (int ii : inputs.keys)
{
	real e = 0;

	RootObject h = RootGetObject(topDir + inputs[ii] + "/diagnostics.root", "common/plots per RP set/" + rpSet + "/chi^2 norm, lin, selected", error=false);
	if (h.valid)
		e = h.rExec("GetEntries");

	//write(e);
	
	draw((ii, e), nullpen, inputs[ii], mCi+2pt+input_pens[ii]);
}
